...
class TicketModal(ui.Modal, title='Crea Ticket'):
    ...
    def __init__(self):
        super().__init__()
        self.add_item(
            ui.TextInput(label='Oggetto',
                         placeholder='Inserisci oggetto ticket...'))
        self.add_item(
            ui.TextInput(label='Descrizione',
                         style=discord.TextStyle.paragraph))
        self.add_item(
            ui.TextInput(label='Priorità', placeholder='alta/media/bassa'))
    ...
    async def on_submit(self, interaction: discord.Interaction):
        ...
        if not category:
            await interaction.response.send_message(
                "Errore: categoria ticket non trovata!", ephemeral=True)
            return
        ...
        colors = {
            'alta': discord.Color.red(),
            'media': discord.Color.orange(),
            'bassa': discord.Color.green()
        }

        embed = discord.Embed(title=f"Ticket #{ticket_count}",
        ...
        class TicketButtons(discord.ui.View):
            ...
            @discord.ui.button(label="Chiudi Ticket",
                               style=discord.ButtonStyle.danger)
            async def close_ticket(self, interaction: discord.Interaction,
                                   button: discord.ui.Button):
                if staff_role not in interaction.user.roles:
                    await interaction.response.send_message(
                        "Solo lo staff può chiudere i ticket!", ephemeral=True)
                    return
                ...
                if transcript_channel:
                    ...
                    await transcript_channel.send(
                        f"Transcript del ticket {channel.name}", file=file)

                await interaction.response.send_message(
                    "Ticket chiuso e trascrizione salvata nel canale transcript."
                )
                await asyncio.sleep(5)
                await channel.delete()

            @discord.ui.button(label="Assegna",
                               style=discord.ButtonStyle.primary)
            async def assign_ticket(self, interaction: discord.Interaction,
                                    button: discord.ui.Button):
                if staff_role not in interaction.user.roles:
                    await interaction.response.send_message(
                        "Solo lo staff può assegnare i ticket!",
                        ephemeral=True)
                    return
                ...
                await interaction.response.send_message(
                    f"Ticket assegnato a {interaction.user.mention}")
        ...
        await channel.send(content=priority_mention[priority],
                           embed=embed,
                           view=view)
        await interaction.response.send_message(
            f"Ticket creato! Controlla {channel.mention}", ephemeral=True)
...
async def check_inactive_tickets():
    while True:
        ...
        for (channel_id, ) in inactive_tickets:
            ...
            if channel:
                await channel.send(
                    "⚠️ Questo ticket verrà chiuso automaticamente per inattività tra 24 ore."
                )
...
async def stats(ctx):
    ...
    embed = discord.Embed(title="Statistiche Ticket",
...
async def mytickets(ctx):
    ...
    c = conn.cursor()
    c.execute(
        '''SELECT ticket_id, status, created_at FROM tickets WHERE user_id = ?''',
        (ctx.author.id, ))
    tickets = c.fetchall()
    conn.close()
...
try:
    ...
    if e.status == 429:
        print(
            "I server Discord hanno negato la connessione per troppe richieste"
        )
    else:
        raise e
